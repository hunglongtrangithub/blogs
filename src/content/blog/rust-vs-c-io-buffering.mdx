...
But does `libc::read` or `libc::write` initialize buffers? If not, how do functions in `libc` like `printf`, `fwrite`, `fread`, and many others have buffered IO? With these questions flying around in my head, I decided to look at [`glibc`](https://github.com/bminor/glibc)'s source code to find out myself 🫠. Brace yourself. It's gonna be quite a journey.
...
From looking around the codebase, I came across this [`fileops.c`](https://github.com/bminor/glibc/blob/master/libio/fileops.c) and [`genops.c`](https://github.com/bminor/glibc/blob/master/libio/genops.c) which contain various functions for `FILE` struct. I have discovered things like [`_IO_default_allocate`](https://github.com/bminor/glibc/blob/master/libio/genops.c#L503), [`_IO_setb`](https://github.com/bminor/glibc/blob/master/libio/genops.c#L354), [`_IO_doallocbuf`](https://github.com/bminor/glibc/blob/master/libio/genops.c#L368), and [`_IO_file_doallocate`](https://github.com/bminor/glibc/blob/master/libio/filedoalloc.c#L67), which look like they are associated with buffer initialization.
...
[`__io_vtables`](https://github.com/bminor/glibc/blob/master/libio/vtables.c#L92) holds all functions for different kinds of IO. I guess this is how people implement polymorphism for `FILE` in C. Every `FILE` pointer is created so that it sits next to a pointer to a corresponding v-table in memory.
